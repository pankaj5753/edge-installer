#!/usr/bin/env bash
# Optional, standalone helper: checks whether this host has Docker Engine
# (>= 24), the Docker Compose plugin (>= 2.20), and OpenSSL, and offers to
# install anything missing via Docker's official apt/dnf repo (not the
# distro's, which is often too old to meet the >= 24 requirement).
#
# Entirely separate from install.sh/preflight.sh - run this first, by
# hand, only if you're not sure this host is ready and it has internet
# access. install.sh's own preflight check is unaffected either way.
#
